...
btn.addEventListener('click', newCitation)
let = iCitation = 0
let citations = [
...
function newCitation() {
    ...
    cardText.textContent = citations[nombreAlea]
    console.log(nombreAlea)
    if (nombreAlea != iCitation) {
        iCitation = nombreAlea;
        cardText.textContent = citations[iCitation]
    } else {
        newCitation()
    }
}
